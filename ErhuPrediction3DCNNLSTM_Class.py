...
def body_segment(img, frame):
    ...
    with mp_holistic.Holistic( min_detection_confidence=0.3, min_tracking_confidence=0.3) as holistic:
        ...
        x_L1  = round(results.pose_landmarks.landmark[mp_holistic.PoseLandmark.LEFT_SHOULDER].x * image_width) + round(length_of_leftarm_boundaries_1[0] * var_multiplier)
        x_L2  = round(results.pose_landmarks.landmark[mp_holistic.PoseLandmark.LEFT_SHOULDER].y * image_height) - round(length_of_leftarm_boundaries_1[1] * var_multiplier)
        x_L3  = round(results.pose_landmarks.landmark[mp_holistic.PoseLandmark.LEFT_SHOULDER].x * image_width) - round(length_of_leftarm_boundaries_2[0] * var_multiplier)
        x_L4  = round(results.pose_landmarks.landmark[mp_holistic.PoseLandmark.LEFT_SHOULDER].y * image_height) + round(length_of_leftarm_boundaries_2[1] * var_multiplier)
        # Left hand thumb rectangle point
        x_L1a = round(results.pose_landmarks.landmark[mp_holistic.PoseLandmark.RIGHT_THUMB].x * image_width) + round(length_of_hand_boundaries * var_multiplier)
        x_L2a = round(results.pose_landmarks.landmark[mp_holistic.PoseLandmark.RIGHT_THUMB].y * image_height) - round(length_of_hand_boundaries * var_multiplier)
        x_L3a = round(results.pose_landmarks.landmark[mp_holistic.PoseLandmark.RIGHT_THUMB].x * image_width) - round(length_of_hand_boundaries * var_multiplier)
        x_L4a = round(results.pose_landmarks.landmark[mp_holistic.PoseLandmark.RIGHT_THUMB].y * image_height) + round(length_of_hand_boundaries * var_multiplier)
        ...
        x_l1_rightHandArm = round(results.pose_landmarks.landmark[mp_holistic.PoseLandmark.RIGHT_SHOULDER].x * image_width) + round(length_of_rightarm_boundaries_1[0] * var_multiplier)
        x_L2_rightHandArm = round(results.pose_landmarks.landmark[mp_holistic.PoseLandmark.RIGHT_SHOULDER].y * image_height) - round(length_of_rightarm_boundaries_1[1] * var_multiplier)
        x_R1_rightHandArm = round(results.pose_landmarks.landmark[mp_holistic.PoseLandmark.RIGHT_WRIST].x * image_width) - round(length_of_rightarm_boundaries_2[0] * var_multiplier)
        x_R2_rightHandArm = round(results.pose_landmarks.landmark[mp_holistic.PoseLandmark.RIGHT_WRIST].y * image_height) + round(length_of_rightarm_boundaries_2[1] * var_multiplier)
...
def main_predict(video_path):
    model_bow        = load_model('model/model_bow/model-009.h5')
    model_left_hand  = load_model('model/model_left_hand/model-009.h5')
    model_left_arm   = load_model('model/model_left_arm/model-023.h5')
    model_right_hand = load_model('model/model_right_hand/model-011.h5')
    model_right_arm  = load_model('model/model_right_arm/model-009.h5')
    model_qin_thong  = load_model('model/model_qin_thong/model-014.h5')
    model_qin_zhen   = load_model('model/model_qin_zhen/model-011.h5')
    model_bow.summary()
    model_left_hand.summary()
    model_left_arm.summary()
    model_right_hand.summary()
    model_right_arm.summary()
    model_qin_thong.summary()
    model_qin_zhen.summary()
    ...
    segment_image.load_model("model/model_segmentation/mask_rcnn_model.081-0.129956.h5")
        ...
        if (frame_number > frame_start) :
            if (limit_counter_body < limit_sample) or (limit_counter_erhu < limit_sample) or (limit_counter_bow < limit_sample):
                    ...
                    for i in range(segLeng):
                        ...
                        if mask.shape[0] == seg_mask['masks'].shape[0] and mask.shape[1] == seg_mask['masks'].shape[1]:
                            ...
                            if label == 'erhu':
                                ...
                                knees_shoulder_distance, degree_ear_face, degree_body, degree_shoulder  = body_segment(image, mask)
                ...
                for img_original, ori_right_hand_point, ori_right_arm_point, ori_left_arm_point, all_body_point, bow_line, erhu_line in \
                        zip(x_test_video_original, x_test_right_hand_point, x_test_right_arm_point, x_test_left_arm_point, x_test_all_body_point, x_test_bow_line_point, x_test_erhu_line_point):
                    warning_mess = []
                    ...
                    if degree_ear_face >= K_var or degree_ear_face < (0-K_var):
                        ...
                        warning_mess.append(["E11", str(degree_ear_face), 'Head Position', str(degree_ear_face), 'E11-Head position not normal (to L or R)'])
                    ...
                    if degree_body > (90+K_var) or degree_body < (90-K_var):
                        ...
                        warning_mess.append(["E14", str(degree_body), 'Body Position', str(degree_body), 'E14-Need to seat straight (to L or R)'])
                    ...
                    if degree_shoulder > ((X_var/90)*100):
                        draw_res_bow.text((10, 110), E13_classname + ':' + str(degree_shoulder), font=result_font, fill=(b, g, r, a))
                        warning_mess.append(["E13", str(degree_shoulder), 'Shoulder Position', str(degree_shoulder), 'E13-Right shoulder too hight '])
                        draw_res_bow.rectangle(body_coordinate, outline="blue", fill=None, width=4)
                    elif degree_shoulder < (0-((X_var/90)*100)):
                        draw_res_bow.text((10, 110), E12_classname + ':' + str(degree_shoulder), font=result_font, fill=(b, g, r, a))
                        warning_mess.append(["E12", str(degree_shoulder), 'Shoulder Position', str(degree_shoulder), 'E12-Left shoulder too hight'])
                        draw_res_bow.rectangle(body_coordinate, outline="blue", fill=None, width=4)
                    else:
                    ...
                    if rightArm_E31 >= P_var:
                        draw_res_bow.text((10, 170), rightArm_E31_ClassName + ':' + str(rightArm_E31), font=result_font, fill=(b, g, r, a))
                        warning_mess.append(["E31", str(rightArm_E31), 'Right Hand Arm Position', str(rightArm_E31), 'E31-Wrong RH thumb position'])
                        draw_res_bow.rectangle(ori_right_hand_rectangle_shape, outline="blue", fill=None, width=4)
                    elif rightArm_E32 >= P_var:
                        draw_res_bow.text((10, 170), rightArm_E32_ClassName + ':' + str(rightArm_E32), font=result_font, fill=(b, g, r, a))
                        warning_mess.append(["E32", str(rightArm_E32), 'Right Hand Arm Position', str(rightArm_E32), 'E32-Wrong RH index finger position'])
                        draw_res_bow.rectangle(ori_right_hand_rectangle_shape, outline="blue", fill=None, width=4)
                    elif rightArm_E33 >= P_var:
                        draw_res_bow.text((10, 170), rightArm_E33_ClassName + ':' + str(rightArm_E33), font=result_font, fill=(b, g, r, a))
                        warning_mess.append(["E33", str(rightArm_E33), 'Right Hand Arm Position', str(rightArm_E33), 'E33-Wrong RH middle or ring finger position'])
                        draw_res_bow.rectangle(ori_right_hand_rectangle_shape, outline="blue", fill=None, width=4)
                    elif rightArm_E34 >= P_var:
                        draw_res_bow.text((10, 170), rightArm_E34_ClassName + ':' + str(rightArm_E34), font=result_font, fill=(b, g, r, a))
                        warning_mess.append(["E34", str(rightArm_E34), 'Right Hand Arm Position', str(rightArm_E34), 'E34-Right wrist position too inward'])
                        draw_res_bow.rectangle(ori_right_arm_rectangle_shape, outline="blue", fill=None, width=4)
                    elif rightArm_E35 >= P_var:
                        draw_res_bow.text((10, 170), rightArm_E35_ClassName + ':' + str(rightArm_E35), font=result_font, fill=(b, g, r, a))
                        warning_mess.append(["E35", str(rightArm_E35), 'Right Hand Arm Position', str(rightArm_E35), 'E35-Right wrist position too outward'])
                        draw_res_bow.rectangle(ori_right_arm_rectangle_shape, outline="blue", fill=None, width=4)
                    else:
                    ...
                    if leftArm_E21 >= P_var:
                        draw_res_bow.text((10, 230), leftArm_E21_ClassName + ':' + str(leftArm_E21), font=result_font, fill=(b, g, r, a))
                        warning_mess.append(["E21", str(leftArm_E21), 'Left Hand Arm Position', str(leftArm_E21), 'E21-Left elbow too Hight'])
                        draw_res_bow.rectangle(ori_left_arm_rectangle_shape, outline='blue', fill=None, width=4)
                    elif leftArm_E22 >= P_var:
                        draw_res_bow.text((10, 230), leftArm_E22_ClassName + ':' + str(leftArm_E22), font=result_font, fill=(b, g, r, a))
                        warning_mess.append(["E22", str(leftArm_E22), 'Left Hand Arm Position', str(leftArm_E22), 'E22-Left elbow too Low'])
                        draw_res_bow.rectangle(ori_left_arm_rectangle_shape, outline='blue', fill=None, width=4)
                    elif leftArm_E23 >= P_var:
                        draw_res_bow.text((10, 230), leftArm_E23_ClassName + ':' + str(leftArm_E23), font=result_font, fill=(b, g, r, a))
                        warning_mess.append(["E23", str(leftArm_E23), 'Left Hand Arm Position', str(leftArm_E23), 'E23-Left elbow and wrist in a line'])
                        draw_res_bow.rectangle(ori_left_arm_rectangle_shape, outline='blue', fill=None, width=4)
                    else:
                        draw_res_bow.text((10, 230), leftArm_Normal_ClassName, font=result_font, fill=(b, g, r, a))
                        warning_mess.append(["Left Arm Position", "Normal", 'Left Hand Arm Position', str(1.0), 'Normal'])
                    ...
                    if is_orthogonal == False:
                        ...
                        warning_mess.append(["Bow Erhu Position", "E41", "Bow Erhu Position", str(1.0), "E41-Pole tilt to left - Bow hair and string must be orthogonal"])
                    else:
                        warning_mess.append(["Bow Erhu Position", "Normal", "Bow Erhu Position", str(1.0),  "Normal"])
                    resize_img = (256, 256)
                    img_original = np.array(img_pil.resize(resize_img))
    ...
    time_save   = end_now.strftime("%H%M%S")
    end_time    = end_now.strftime("%H:%M:%S")
    print("Start Time =", start_time)
    print("End Time =", end_time)
    np.savez_compressed("result/output_"+datetoday+'_'+time_save+'_'+filename+"_npy", output_array)
    videoInput.release()
