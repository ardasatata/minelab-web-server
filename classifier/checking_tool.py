...
def check_player_img_postition(img):
    ...
    with mp_holistic_checker.Holistic(min_detection_confidence=0.3, min_tracking_confidence=0.3) as holistic:
        ...
        x_LK = round(results.pose_landmarks.landmark[mp_holistic_checker.PoseLandmark.LEFT_KNEE].x * image_width)
        y_LK = round(results.pose_landmarks.landmark[mp_holistic_checker.PoseLandmark.LEFT_KNEE].y * image_height)
        x_RK = round(results.pose_landmarks.landmark[mp_holistic_checker.PoseLandmark.RIGHT_KNEE].x * image_width)
        y_RK = round(results.pose_landmarks.landmark[mp_holistic_checker.PoseLandmark.RIGHT_KNEE].y * image_height)
        x_lf_shoulder = round(
            results.pose_landmarks.landmark[mp_holistic_checker.PoseLandmark.LEFT_SHOULDER].x * image_width)
        y_lf_shoulder = round(
            results.pose_landmarks.landmark[mp_holistic_checker.PoseLandmark.LEFT_SHOULDER].y * image_height)
        x_rh_shoulder = round(
            results.pose_landmarks.landmark[mp_holistic_checker.PoseLandmark.RIGHT_SHOULDER].x * image_width)
        y_hip = round(results.pose_landmarks.landmark[mp_holistic_checker.PoseLandmark.LEFT_HIP].y * image_height)
        ...
        image = cv2.line(image, (x_middle_shoulder, y_middle_shoulder), (x_middle_shoulder, y_middle_shoulder + 100), (255, 0, 0), thickness=3)
